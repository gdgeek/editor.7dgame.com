...
let activeGuardPromise = null;

async function hasUnsavedEntityChanges( editor ) {
...
function getSaveConfirmMessage( editor ) {
...
function replayNavigationClick( element, state ) {
...
async function runEntitySaveGuard( editor, onProceed, onCancel ) {

	if ( activeGuardPromise ) return activeGuardPromise;

	activeGuardPromise = new Promise( async ( resolve ) => {
...
function getLeftNavClickableTarget( target ) {

	if ( ! target || typeof target.closest !== 'function' ) return null;

	const candidate = target.closest( 'a[href], [role="menuitem"], .el-menu-item, .sidebar-item, .nav-item, .menu-item' );
...
function bindParentNavigationGuard( editor ) {
	...
	const state = { skipNextClick: false };
