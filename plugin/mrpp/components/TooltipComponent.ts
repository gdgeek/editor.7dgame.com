...
class TooltipComponent {
  ...
  constructor(editor: MrppEditor, object: THREE.Object3D, component: MrppComponent) {
    ...
    const traverseChildren = (node: THREE.Object3D): void => {
    ...
    this.editor.signals.objectSelected.add((selectedObject: THREE.Object3D | null) => {
   ...
   * 创建默认的 Tooltip 组件数据。
   */
  static Create(editor: MrppEditor): MrppComponent {
   ...
   * 获取指定类型的已选 UUID。
   */
  getSelectedUUID(type: string): string {
   ...
   * 更新目标位置参数。
   */
  updatePosition(type: string): void {
