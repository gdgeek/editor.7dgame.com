 ...
 */
function ensureKTX2( state: KTX2State, manager: any, editor: MrppEditor ): any {
 ...
 */
function applyLoaderPatches( editor: MrppEditor ): void {

	const loader: any = editor.loader;

	// Shared mutable state for the lazy-initialized KTX2Loader
	const ktx2State: KTX2State = { ktx2Loader: null };
	...
	loader.ensureKTX2 = function ( manager: any ): any {
	...
	loader.loadFiles = function ( files: any, filesMap: any ): any {
		...
		try {
			...
			if ( files && files.length > 0 &&
				files.some( ( f: any ) => /\.(ktx2|glb|gltf)$/i.test( f.name ) ) ) {
	...
	loader.loadFile = function ( file: any, manager: any ): any {
