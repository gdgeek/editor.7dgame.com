 ...
 */
function computeEnhancedBoundingBox( object: THREE.Object3D ): THREE.Box3 {
 ...
 */
function computeMultiSelectionBoundingBox( objects: THREE.Object3D[] ): THREE.Box3 {
 ...
 */
function applyViewportPatches( editor: MrppEditor ): void {

	// ─── Internal state namespace ───────────────────────────────────

	const state: any = {
		multipleObjectsTransformOnDown: [] as any[],
	...
	const multiSelectGroup: any = new THREE.Group();
	...
	function updateMultiSelectionTransforms( forceUpdateCenter: boolean ): void {
		...
		try {
			...
			if ( object !== null && object !== editor.scene && object !== editor.camera ) {
				...
				if ( selectedObjects.length > 1 ) {
					...
					multiSelectGroup.userData.onPositionChange = function (): void {
					...
					multiSelectGroup.userData.onRotationChange = function (): void {
					...
					multiSelectGroup.userData.onScaleChange = function (): void {
	...
	editor.signals.transformModeChanged.add( function ( mode: string ) {

		state.currentTransformMode = mode;

	} );

	editor.signals.objectChanged.add( function ( object: any ) {
	...
	editor.signals.multipleObjectsTransformChanged.add( function ( object: any ) {
	 ...
	 */
	state.handleMultiSelectChange = function ( transformControls: any, box: THREE.Box3, editorSignals: any ): boolean {
		...
		editorSignals.refreshSidebarObject3D.dispatch( object );
		editorSignals.multipleObjectsTransformChanged.dispatch( object );
	 ...
	 */
	state.handleMultiSelectMouseDown = function ( transformControls: any ): boolean {
	 ...
	 */
	state.handleMultiSelectMouseUp = function ( transformControls: any, SetPositionCommand: any, SetRotationCommand: any, SetScaleCommand: any ): boolean {
