...
/**
 * Deep-copies components / commands arrays and regenerates all UUIDs
 * so the cloned object has unique identifiers.
 */
function copyComponentsWithNewUUIDs( source, target ) {
...
/**
 * Detects the replaceable resource type of an object.
 * Returns the type string ('polygen','voxel','picture','audio','video') or ''.
 */
function detectReplaceableType( object ) {
...
/**
 * Injects MRPP-specific edit functionality into the Edit menu.
 *
 * - Multi-select clone (with components/commands copy and UUID regeneration)
 * - Resource replacement (replace-resource message handling)
 * - Multi-select delete
 * - Ctrl+C/V copy/paste, Del/Backspace keyboard shortcuts
 *
 * @param {object} editor          – the Editor instance (already patched)
 * @param {object} editMenuOptions – the UIPanel `options` container of the Edit menu
 */
function injectMrppEditMenu( editor, editMenuOptions ) {
	...
	cloneOption.onClick( function () {
		...
		for ( let i = 0; i < selectedObjects.length; i ++ ) {
			...
			if ( original.animations && original.animations.length > 0 ) {
				...
				object.animations = original.animations.map( clip => clip.clone() );
