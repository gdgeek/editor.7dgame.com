...
function MenubarCommand(editor: MrppEditor): InstanceType<typeof UIPanel> {
    ...
    Object.keys(commandTypes).forEach(function(type) {
        ...
        typeRow.onClick(function(event: any) {
                ...
                // 多选对象处理
                    ...
                    for (let i = 0; i < selectedObjects.length; i++) {
                        ...
                        if (command !== undefined) {
                            const cmd = new AddCommandCommand(editor, object as any, command);
                            editor.execute(cmd);
                        }
                    }
                ...
                if (command !== undefined) {
                const cmd = new AddCommandCommand(editor, editor.selected as any, command);
                    editor.execute(cmd);
