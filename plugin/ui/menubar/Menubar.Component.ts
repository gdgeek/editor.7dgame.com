...
import { UIPanel, UIRow, UIHorizontalRule, UIButton, UISelect, UIText } from '../../../three.js/editor/js/libs/ui.js';
...
function MenubarComponent(editor: MrppEditor): InstanceType<typeof UIPanel> {
    ...
    // 存储组件菜单项的引用
    const typeRows: Record<string, any> = {};
    ...
    Object.keys(componentTypes).forEach(function(type) {
        ...
        typeRow.onClick(function(event: any) {
            if (editor.selected !== null) {
                ...
                if (selectedObjects.length > 1) {
                    ...
                    if (mutuallyExclusiveTypes.includes(type)) {
                        // 检查是否有对象已经存在互斥组件
                        let objectsWithExclusiveComponents: string[] = [];
                        ...
                        for (let i = 0; i < selectedObjects.length; i++) {
                            ...
                            if (component !== undefined) {
                                const command = new AddComponentCommand(editor, object as any, component);
                    ...
                    // 单选对象处理（原有逻辑）
                // 确保对象有components属性
                if ((editor.selected as any).components === undefined) {
                    (editor.selected as any).components = [];
                }
                ...
                if (mutuallyExclusiveTypes.includes(type)) {
                    ...
                    for (let i = 0; i < (editor.selected as any).components.length; i++) {
                        const compType = (editor.selected as any).components[i].type;
                        if (mutuallyExclusiveTypes.includes(compType) && compType !== type) {
                ...
                if (component !== undefined) {
                    const command = new AddComponentCommand(editor, editor.selected as any, component);
                    editor.execute(command);
    ...
    function updateMutuallyExclusiveOptions() {
        if (editor.selected !== null) {
            const sel = editor.selected as any;
            // 确保对象有components属性
            if (sel.components === undefined) {
                sel.components = [];
            }

            // 检查当前选中对象已有的组件
            const existingComponents = sel.components;
            ...
            mutuallyExclusiveTypes.forEach(function(type) {
                if (existingExclusiveType && type !== existingExclusiveType) {
                    ...
                    typeRows[type].onClick(function(event: any) {
