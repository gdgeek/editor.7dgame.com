...
import { UIPanel, UIRow, UIButton, UIInput, UISelect, UIText } from '../../../three.js/editor/js/libs/ui.js';
import { UIBoolean } from '../../../three.js/editor/js/libs/ui.three.js';
import { ScreenshotUtils } from '../../utils/ScreenshotUtils.js';

function SidebarScreenshot(editor: any): InstanceType<typeof UIPanel> {
    ...
    function captureScreenshot() {
        ...
        try {
            ...
            if (shadows !== undefined) {
                (tempRenderer as any).shadowMap.enabled = shadows;
            }

            // 阴影类型
            const shadowType = editor.config.getKey('project/renderer/shadowType');
            if (shadowType !== undefined) {
                (tempRenderer as any).shadowMap.type = shadowType;
            }
            ...
            if (shouldShowGrid) {
                ...
                (grid1.material as any).color.setHex(0x888888);
                (grid1.material as any).vertexColors = false;
                grid.add((grid1 as any));

                const grid2 = new THREE.GridHelper(30, 6, 0x222222);
                (grid2.material as any).color.setHex(0x222222);
                (grid2.material as any).depthFunc = THREE.AlwaysDepth;
                (grid2.material as any).vertexColors = false;
                grid.add((grid2 as any));
            ...
            // 创建下载链接
            const dataURL = (tempRenderer.domElement as any).toDataURL(mimeType, quality);
            ...
            console.log('正在显示错误通知: ' + (error as any).message);
            editor.showNotification(strings.getKey('menubar/screenshot/error/capture_failed') + ': ' + (error as any).message, true);
        }
    }
