import { UIPanel, UIBreak, UIRow, UIText, UIButton, UISelect, UINumber, UIDiv, UICheckbox } from '../../../three.js/editor/js/libs/ui.js';
import * as THREE from 'three';
import { SetValueCommand } from '../../../three.js/editor/js/commands/SetValueCommand.js';

function SidebarMedia(editor: any): InstanceType<typeof UIPanel> {
    const strings = editor.strings;
    const signals = editor.signals;

    const mediaPlayers = new Map();

    function getMediaPlayer(object: any) {
    ...
    function getMediaType(object: any) {
    ...
    // 从对象获取媒体URL
    function getMediaUrl(object: any) {
    ...
    // 更新对象的userData和UI
    function updateObjectUserData(object: any, key: any, value: any) {
        ...
        if (userDataTextarea) {
            try {
                (userDataTextarea as any).value = JSON.stringify(userData, null, '  ');
    ...
    function pauseMedia(object: any) {
        const player = getMediaPlayer(object);
        if (!player) return;
    ...
    function setLoop(object: any, loop: any) {
        const player = getMediaPlayer(object);
        if (!player) return;
    ...
    function setMuted(object: any, muted: any) {
        const player = getMediaPlayer(object);
    ...
    function setVolume(object: any, volume: any) {
        const player = getMediaPlayer(object);
        if (!player) return;
    ...
    function setPlaybackRate(object: any, rate: any) {
        const player = getMediaPlayer(object);
        if (!player) return;
    ...
    // 更新视频的媒体纹理
    function updateVideoTexture(object: any) {
        const player = getMediaPlayer(object);
        if (!player || player.type !== 'video') return;
    ...
    function updateTimeDisplay(object: any) {
        if (!object || object !== currentObject) return;
    ...
    function cleanupMediaPlayer(object: any) {
        if (mediaPlayers.has(object.uuid)) {
    ...
    let playPauseCheckbox: any = null;
    let currentTimeText: any = null;
    let volumeNumber: any = null;
    let rateNumber: any = null;
    let loopCheckbox: any = null;
    let mutedCheckbox: any = null;
    ...
    function createMediaControls(object: any) {
        controlsPanel.clear();
