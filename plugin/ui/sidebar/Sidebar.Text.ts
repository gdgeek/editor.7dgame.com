import { UIPanel, UIRow, UITextArea, UIText, UIInput, UINumber, UIColor, UIButton } from '../../../three.js/editor/js/libs/ui.js';
import { SetValueCommand } from '../../../three.js/editor/js/commands/SetValueCommand.js';
import { createTextMesh } from '../../utils/TextUtils.js';

function SidebarText(editor: any): { container: InstanceType<typeof UIPanel>; update: () => void } {
	...
	function updateVisuals() {
		...
		(container as any).updateTextObject(editor.selected, getUiData());
	}

	// 提交（保存 + 产生历史记录）
	function commitChange(key: any, value: any) {
		if (!editor.selected || editor.selected.type !== 'Text') return;
	...
	function clampInput(inputUI: any, min: any, max: any) {
	...
	fontSizeSlider.addEventListener('input', () => {
		fontSizeNumber.setValue((fontSizeSlider.value as any));
		updateVisuals();
	});
	...
	fontSizeNumber.onInput(() => {
		fontSizeSlider.value = (fontSizeNumber.getValue()) as any;
		updateVisuals();
	});
	fontSizeNumber.onChange(() => {
		const val = clampInput(fontSizeNumber, 8, 200);
		fontSizeSlider.value = (val) as any;
	...
	function createAlignGroup(options: any, axis: any): Record<string, any> {
		const buttons: Record<string, any> = {};
		options.forEach((opt: any) => {
			const btn = new UIButton();
			btn.dom.innerHTML = (icons as any)[opt];
			btn.setWidth('45px');
			btn.onClick(() => {
				(currentAlign as any)[axis] = opt;
				updateAlignVisuals(buttons, opt);
				updateVisuals(); 
				commitChange('align', { ...currentAlign }); 
			});
			(buttons as any)[opt] = btn;
		});
		return buttons;
	}

	function updateAlignVisuals(btnMap: any, activeKey: any) {
	...
	alignRow.add(hButtons.left, hButtons.center, (hButtons as any).right);
	
	verticalRow.add(new UIText('').setWidth('90px')); 
	verticalRow.add(vButtons.top, vButtons.middle, (vButtons as any).bottom);
	...
	backgroundOpacityNumber.onInput(() => {
		const val = parseFloat((backgroundOpacityNumber.getValue() as any));
		if (!isNaN(val)) {
			backgroundOpacitySlider.value = (val) as any;
			updateVisuals();
		}
	});
	...
	(container as any).updateTextObject = function (textObject: any, dataOverride: any) {
	...
	// ================= 状态同步逻辑 =================
	function updateUIState(object: any) {
	...
	signals.objectChanged.add(function (object: any) {
		...
		if (object.type === 'Text') {
			...
			(container as any).updateTextObject(object);
		}
	});
