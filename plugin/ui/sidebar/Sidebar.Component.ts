...
function SidebarComponent(editor: any): { container: InstanceType<typeof UIPanel>; update: () => void } {
	...
	function isEntityOnlySelection(objects: any) {
	...
	function update() {
		...
		newComponent.onClick(function() {
			...
			if (selectedObjects.length > 1) {
				...
				// 显示成功消息
				const componentName = (select as any).getSelectedHtml();
				editor.showNotification(
				...
				if (component !== undefined) {
					...
					const successMessage = strings.getKey('menubar/component/success').replace('{0}', (select as any).getSelectedHtml());
		...
		if (isMultiSelect) {
			...
			// 获取下拉选项的DOM元素
			const options = (select.dom as any).options;
		...
		if (isMultiSelect) {
			...
			for (let i = 0; i < selectedObjects.length; i++) {
				...
				const types = (object.components && object.components.length > 0)
					? object.components.map((c: any) => c.type).join(', ')
	...
	// 更新互斥组件选项的可用状态
	function updateMutuallyExclusiveOptions(select: any, components: any) {
