...
function SidebarCommand(editor: any): { container: InstanceType<typeof UIPanel>; update: () => void } {
	...
	function update() {
		...
		if (!isMultiSelect && object) {
			...
			newCommand.onClick(function() {
				...
				if (command !== undefined) {
					...
					const successMessage = strings.getKey('menubar/command/success').replace('{0}', (select as any).getSelectedHtml());
		...
		// 多选对象模式
			...
			newCommand.onClick(function() {
				// 多选时按下拉选择的类型执行批量添加
				const chosenType = select.getValue();
				const validObjects = selectedObjects.filter((obj: any) => obj.type && obj.type.toLowerCase() === 'entity');
				const invalidObjects = selectedObjects.filter((obj: any) => !obj.type || obj.type.toLowerCase() !== 'entity');

				// 如果没有有效对象，显示错误并返回
				if (validObjects.length === 0) {
					if (invalidObjects.length > 0) {
						const names = invalidObjects.map((obj: any, i: any) => obj.name || strings.getKey('sidebar/command/object_name_with_index').replace('{0}', i+1));
			...
			for (let i = 0; i < selectedObjects.length; i++) {
				...
				const types = (object.commands && object.commands.length > 0)
					? object.commands.map((c: any) => c.type).join(', ')
