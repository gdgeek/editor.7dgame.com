...
function SidebarAnimation(editor: any): { container: InstanceType<typeof UIPanel>; update: (object: any) => void } {
	...
	function getAnimationState(object: any) {
	...
	let updateRequestId: any = null;

	function createAnimationControls(object: any, animations: any) {
		...
		const options = {};
		animations.forEach((anim: any) => (options as any)[anim.name] = anim.name);
		...
		const createIconButton = (html: any) => {
		...
		(progressBar.dom as any).type = 'range';
		(progressBar.dom as any).min = 0;
		(progressBar.dom as any).max = 1;
		(progressBar.dom as any).step = 0.001;
		...
		function getAction() {
			...
			const clip = animations.find((a: any) => a.name === name);
			if (!clip) return null;
		...
		function refreshUIStatus() {
			...
			if (duration < EPSILON) {
				...
				(progressBar.dom as any).disabled = true;
				(progressBar.dom as any).value = 1;
				...
				(progressBar.dom as any).disabled = false;
				const progress = action.time / duration;
				(progressBar.dom as any).value = isNaN(progress) ? 0 : progress;
		...
		function updateUI() {
			...
			if (action && action.getClip().duration >= EPSILON && state.isPlaying) {
				const duration = action.getClip().duration;
				(progressBar.dom as any).value = action.time / duration;
				timeDisplay.setValue(`${action.time.toFixed(2)}s / ${duration.toFixed(2)}s`);
		...
		progressBar.dom.addEventListener('input', () => {
			...
			if (action && action.getClip().duration >= EPSILON) {
				...
				action.time = (progressBar.dom as any).value * duration;
				mixer.update(0);
				timeDisplay.setValue(`${action.time.toFixed(2)}s / ${duration.toFixed(2)}s`);
			}
		});
