 ...
 */
class DialogUtils {
    static showMessage(message: string, isError: boolean = false, duration: number = 3000): HTMLElement {
        // 创建临时通知元素
    ...
    static closeNotification(notification: HTMLElement): void {
        notification.style.opacity = '0';
        setTimeout(function() {
    ...
    static showConfirm(message: string, onConfirm: Function | null, onCancel: Function | null, event: Event | undefined, isError: boolean = false): HTMLElement {
         // 获取多语言字符串
        ...
        const clickOutsideHandler = function(e: MouseEvent) {
            if (!confirmNotification.contains(e.target as Node) && e.target !== event?.target) {
     ...
     */
    static showSceneSaveDialog(message: string, onYes: (() => void | Promise<void>) | null, onNo: (() => void | Promise<void>) | null, onClose: (() => void | Promise<void>) | null): HTMLElement {
        const existing = document.querySelector('.scene-save-dialog-overlay');
        if (existing) {
            existing.remove();
        }

        const setImportantStyle = function (element: HTMLElement, property: string, value: string): void {
            element.style.setProperty(property, value, 'important');
        };

        const bindHoverStyles = function (element: HTMLElement, normalStyles: Record<string, string>, hoverStyles: Record<string, string>): void {
            if (!element) return;

            const applyStyles = function (styles: Record<string, string>): void {
        ...
        const runAndClose = async (cb: (() => void | Promise<void>) | null): Promise<void> => {
            try {
                if (cb) await cb();
            } finally {
                closeDialog();
    ...
    static closeConfirm(confirmNotification: HTMLElement): void {
        confirmNotification.style.opacity = '0';
        confirmNotification.style.transform = 'translateY(4px)';
        setTimeout(function() {
