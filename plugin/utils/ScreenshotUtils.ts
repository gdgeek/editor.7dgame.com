...
class ScreenshotUtils {
	static captureSound: HTMLAudioElement | null = null;

	static initSound(): void {
		if (ScreenshotUtils.captureSound === null) {
			ScreenshotUtils.captureSound = new Audio();
			ScreenshotUtils.captureSound.src = '../../plugin/utils/screenshot.mp3';
			ScreenshotUtils.captureSound.load();
		}
	}

	static createAnimationElements(_previewContainer: HTMLElement | null = null): HTMLElement {
		// 已经存在则不重复创建
		const existing = document.getElementById('screenshot-animation-container');
		if (existing) {
			return existing;
		}
	...
	static playAnimation(imageDataURL: string, previewContainer: HTMLElement | null = null, callback: (() => void) | null = null): void {
					...
					if (!previewContainer && callback && typeof callback === 'function') {
						...
						setTimeout(() => {
							if (previewContainer) {
								...
								// 计算从屏幕中心到预览区域的位置变换
								const translateX = ((centerX / viewportWidth) * 200) - 100;
								const translateY = ((centerY / viewportHeight) * 200) - 100;
